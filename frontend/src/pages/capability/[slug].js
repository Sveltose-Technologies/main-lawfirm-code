...
export default function CapabilityDetail() {
  ...
  useEffect(() => {
    ...
    async function fetchData() {
      setLoading(true);
      try {
        const [catRes, subRes, cmsRes, newsRes, eventRes] = await Promise.all([
          getAllCapabilityCategories(),
          getAllCapabilitySubCategories(),
          getAllCapabilityCategoryCMS(),
          getAllNews(),
        ...
        if (currentCat) {
          ...
          const currentId = Number(currentCat.id);

          // Get all subcategories from database for this category
          const allSubs = subRes?.data || [];
          const dbFilteredSubs = allSubs.filter(
            (sub) => Number(sub.categoryId) === currentId,
          );

          if (cmsRes?.success && cmsRes.data) {
            const matchedCMS = cmsRes.data.find(
              (cms) => Number(cms.categoryId) === currentId,
            );
            if (matchedCMS) {
              setCmsData(matchedCMS);
              // If CMS has specific subcategories, use them, but fallback to DB ones if empty
              if (
                matchedCMS.subcategoryIds &&
                matchedCMS.subcategoryIds.length > 0
              ) {
                const cmsIds = matchedCMS.subcategoryIds.map((id) =>
                  Number(id),
                );
                // Show items that are either in CMS or just newly added to this category
                const combined = allSubs.filter(
                  (sub) =>
                    cmsIds.includes(Number(sub.id)) ||
                    Number(sub.categoryId) === currentId,
                );
                setAreaOfFocusList(combined);
              } else {
                setAreaOfFocusList(dbFilteredSubs);
              }
            } else {
              setAreaOfFocusList(dbFilteredSubs);
            }
          ...
          setRelatedCapabilities(
            categories.filter((c) => Number(c.id) !== currentId).slice(0, 10),
          );
        }
      ...
      <div className="container py-5">
        <div className="row justify-content-end">
          <div className="col-lg-9">
            <h2
              className="display-6 mb-2" // Heading ke niche ka gap kam kiya
              style={{ fontFamily: "serif", fontWeight: "500" }}>
              Areas of Focus
            </h2>

            {/* row-cols-md-2 ke saath g-1 use karne se horizontal aur vertical dono gaps kam ho jayenge */}
            <div className="row row-cols-1 row-cols-md-2 g-1 mb-4">
              {areaOfFocusList.length > 0 ? (
                areaOfFocusList.map((sub) => (
                  <div
                    key={sub.id}
                    className="col d-flex align-items-baseline py-1">
                    {/* py-1 add karne se row ke beech ka space manually control hoga */}
                    <span
                      style={{
                        color: "#a67c33",
                        marginRight: "6px", // Bullet aur text ke beech ka gap bhi 10px se 6px kar diya
                        fontSize: "1.2rem",
                      }}>
                      •
                    </span>
                    <Link
                      ...
                      <a
                        ...
                        style={{
                          ...
                          fontSize: "0.95rem",
                          fontWeight: "500",
                          lineHeight: "1.2", // Line height kam karne se gap aur kam lagega
                        }}>
                        {sub.subcategoryName}
                      </a>
                    </Link>
                ...
                <p className="text-muted">No specific focus areas available.</p>
              )}
            </div>
            <div
              className="mb-5"
              ...
              <div
                dangerouslySetInnerHTML={{
                  __html: cmsData
                    ? cleanHTML(cmsData.content)
                    : cleanHTML(selectedCategory.description),
                }}
              />
            </div>
          </div>
